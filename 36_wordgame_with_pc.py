...
WORDLIST_FILENAME = "others/words_02.txt"
...
'''
    if(word in wordList):
        ...
        wordDict_keys = wordDict.keys()
        hand_keys = hand.keys()
        for key in wordDict_keys:
            if((key not in hand_keys) or (wordDict[key] > hand[key])):
                return False
        return True
    else:
        return False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    if(calculateHandlen(temp_hand) == 0):
...
def playGame(wordList):
    ...
    while(True):
        user_input = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
        if(user_input == "n" or user_input == "r" or user_input == "e"):
            if(user_input == "e"):
                break
